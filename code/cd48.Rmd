...
initial_48 <- list.files(c("data_TCRrecapture_first-try_16022013/cd45-1_DLI_full-4-8_project/ch45_DLI_clones/cd4_data", "data_TCRrecapture_first-try_16022013/cd45-1_DLI_full-4-8_project/ch45_DLI_clones/cd8_data"),full.names = T) %>%
  as.list %>%
  lapply(function(x) read_delim(x) %>% mutate(sample.id = x)) %>%
  rbindlist

initial.long.48 <-initial_48 %>% mutate(time.point = str_split(sample.id, "_", simplify = T)[, 5],
...
head(initial.long.48)

initial_full <- list.files(c("data_TCRrecapture_first-try_16022013/cd45-1_DLI_full-4-8_project/ch45_DLI_clones/F1_data"),full.names = T) %>%
  as.list %>%
...
initial.long.full <-initial_full %>% mutate(time.point = str_split(sample.id, "_", simplify = T)[, 5],
                                      ...
                                      #subset = str_split(sample.id, "_", simplify = T)[, 12],
                                sample.id = stringr::str_extract(sample.id, pattern = "(?<=_data/)(.*?)(?=_)")) %>% 
  select(c('sample.id', 'time.point', 'cloneCount', 'nSeqCDR3', 'bestVGene', 'aaSeqCDR3')) %>% 
  rename(clone.count = cloneCount, nt.seq = nSeqCDR3, aa.seq = aaSeqCDR3)

head(initial.long.full)

# annotate full repertoire by clones from sorted subsets
initial.long.full <- initial.long.full %>% left_join(initial.long.48[, c("sample.id","nt.seq", "bestVGene", "subset")], by=c("sample.id","nt.seq", "bestVGene"))

initial.long.full %>% count(subset)
initial.long.full %>% count(sample.id, subset)

# For Make and Art there is no data sets for CD8/CD4, we will remove them
initial.long.full <- initial.long.full %>% filter(!sample.id %in% c("Make", "Art"))

# upload post repertoire
post_full <- list.files(c("data_TCRrecapture_first-try_16022013/cd45-1_DLI_full-4-8_project/ch45_full48_clones/F1_data"),full.names = T) %>%
  as.list %>%
  lapply(function(x) read_gz(x) %>% mutate(sample.id = x)) %>%
  rbindlist

post.long.full <-post_full %>% mutate(time.point = str_split(sample.id, "_", simplify = T)[, 11],
                                      #time.point = as.numeric(gsub("\\..*","",time.point)),
                                      #subset = str_split(sample.id, "_", simplify = T)[, 12],
                                sample.id = stringr::str_extract(sample.id, pattern = "(?<=_data/)(.*?)(?=_)")) %>% 
  select(c('sample.id', 'time.point', 'cloneCount', 'nSeqCDR3', 'bestVGene', 'aaSeqCDR3')) %>% 
...
post.long.full <- post.long.full %>% filter(!sample.id %in% c("Art", "Baban", "Make", "Meh", "Mish"))

data <- rbind(initial.long.full, post.long.full)

# annotate full repertoire by clones from sorted subsets
data <- data %>% left_join(initial.long.48[, c("sample.id","nt.seq", "bestVGene", "subset")], by=c("sample.id","nt.seq", "bestVGene"))
```
...
data <- data %>%
    mutate(time.point = recode(time.point, 'DLI' = 0, 'p3' = 120, 'p4' = 360))

s <- (unique(data$time.point))
...
data.coord.rbind <- data.alphabeta.df %>%
  ...
  mutate(pre.quantile = factor(pre.quantile, levels = c("Singleton",
                                                       ...
                                                       "Tripleton",
                                                       "Large")),
         subset = factor(subset, levels = c(NA, "CD4", "CD8"), exclude = NULL))
```
