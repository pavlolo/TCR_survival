...
data.wide.tri.l<-lapply(1:length(s), quantiles_notripl)

data.wide.tri <- do.call("rbind", data.wide.tri.l)
rm(data.wide.tri.l)
...
data.wide.tri %>%
  ...
  geom_hline(yintercept = 1000, linetype = "dashed") +
  theme_classic() +
  facet_wrap(~pre.point) +
  theme(legend.position = "bottom")
```


```{r}
data.wide.tri %>%  pivot_longer(as.character(all_of(s)), names_to = "time.point", values_to = "clone.count") %>% filter(pre.quantile != "Missing") %>% group_by(pre.quantile, time.point, sample.id) %>% 
     summarise(pre.rep = n())
...
```{r}
data.wide.tri %>%  pivot_longer(as.character(all_of(s)), names_to = "time.point", values_to = "clone.count") %>% filter(pre.quantile != "Missing") %>% group_by(pre.quantile, time.point, sample.id) %>% 
     summarise(pre.rep = n()) %>% summarise(mean_rep = mean(pre.rep))
...
data.prob %>%
  ...
  facet_grid(factor(pre.point, levels = c('-7', '0', '7', '45'))~factor(time.point, levels = c('0', '7', '15', '45')), scales = "free_y") +
  theme_classic() +
...
data.alphabeta.df %>%
  ...
  facet_grid(pre.point~factor(time.point, levels = c('0', '7', '15', '45'))) +
  theme_classic() +
  theme(aspect = 1, legend.position = "bottom")
```
